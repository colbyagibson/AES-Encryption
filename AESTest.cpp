...
[2, 3, 1, 1]            [s0][s4][s8][s12]            [temp0, temp4, temp8, temp12]
[1, 2, 3, 1]            [s1][s5][s9][s13]            [temp1, temp5, temp9, temp13]
[1, 1, 2, 3]     x      [s2][s6][s10][s14]    =      [temp2, temp6, temp10, temp14]
[3, 1, 1, 2]            [s3][s7][s11][s15]           [temp3, temp7, temp11, temp15]


temp0 = Mul2[s0] ^ Mul3[s1] ^ s2 ^ s3
temp1 = s0 ^ Mul2[s1] ^ Mul3[s2] ^ s3
temp2 = s0 ^ s1 ^ Mul2[s3] ^ Mul3[s3]
temp3 = Mul3[s0] ^ s1 ^ s2 ^ Mul2[s2]
*/
...
void AES_Encrypt(unsigned char* message, unsigned char* key)
{
	...
	AddRoundKey(state, key); //whitening/addRound Key
...
int main()
{
	unsigned char message[] = "This is a message we want to encrypt using AES for Cryptography 399!";
	...
	int original_length = strlen((const char*)message);
	int length_of_padded_message = original_length;

	if (length_of_padded_message % 16 != 0)
		length_of_padded_message = (length_of_padded_message / 16 + 1) * 16;

	unsigned char* padded_message = new unsigned char[length_of_padded_message];
	...
	{
		AES_Encrypt(padded_message + i, key);
	}
